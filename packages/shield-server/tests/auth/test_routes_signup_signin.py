...
Post-§A1.c: ``/v1/auth/sign-up`` is DISABLED under enterprise mode. Tests
that exercise the (open-mode) sign-up route use the ``open_client`` fixture;
tests that just need an authenticated user for downstream behaviour use
...
# --- /v1/auth/sign-up route behaviour (open mode) --------------------- #


def test_signup_creates_user_and_returns_csrf(open_client: TestClient) -> None:
    """Open mode 行为不变 (§A1.c): /sign-up still works and creates a session."""
    r = open_client.post(
        "/v1/auth/sign-up",
...
def test_signup_duplicate_email_returns_409(open_client: TestClient) -> None:
    open_client.post(
...
# --- /v1/auth/sign-in (enterprise mode, bootstrap via storage) -------- #


def test_signin_returns_session(client: TestClient) -> None:
    bootstrap_admin_via_storage(client, email="bob@example.com", password="shield-pw-1")
...
def test_signin_wrong_password_returns_uniform_401(client: TestClient) -> None:
    bootstrap_admin_via_storage(client, email="ck@example.com", password="shield-pw-1")
    client.cookies.clear()
...
def test_get_session_after_signin(client: TestClient) -> None:
    bootstrap_admin_via_storage(client, email="dora@example.com", password="shield-pw-1")
...
def test_sign_out_revokes_session(client: TestClient) -> None:
    bootstrap_admin_via_storage(client, email="ed@example.com", password="shield-pw-1")
    # First GET works.
...
# --- /v1/auth/password/reset/* (enterprise) --------------------------- #
...
def test_password_reset_request_for_known_user_writes_token(client: TestClient) -> None:
    bootstrap_admin_via_storage(client, email="fi@example.com", password="shield-pw-1")
...
def test_password_reset_complete_resets_password_and_revokes_sessions(client: TestClient) -> None:
    bootstrap_admin_via_storage(client, email="gi@example.com", password="shield-pw-1")
    storage = client.app_storage  # type: ignore[attr-defined]
