...
function stepTitle(row: TimelineRow): string {
  if (row.decision === 'BLOCK') return 'Blocked high-risk action';
  if (row.decision === 'ESCALATE') return 'Human review required';
  if (row.decision === 'PASS') return 'Policy check passed';
  return `${row.decision} verdict`;
}

function stepDescription(row: TimelineRow): string {
  if (row.decision === 'BLOCK') {
    return 'The policy engine blocked this tool call before execution.';
  }
  if (row.decision === 'ESCALATE') {
    return 'Risk threshold required an analyst decision before execution.';
  }
  if (row.decision === 'PASS') {
    return 'The action stayed within active policy controls for this run.';
  }
  return 'Recorded policy verdict for this action.';
}
...
function RunStepList({
  ...
  return (
    <div className="border border-border">
      <div className="px-4 py-3 border-b border-border flex items-center justify-between gap-3 flex-wrap">
        <div className="section-label">Decision sequence</div>
